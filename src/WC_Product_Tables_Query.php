<?php
/**
 * File for JazzMan\WCProductTables\WC_Product_Tables_Query.
 *
...
/**
 * Class JazzMan\WCProductTables\WC_Product_Tables_Query
 *
...
class WC_Product_Tables_Query
{
    ...
    {
        if ( ! is_admin()) {
            add_filter('woocommerce_get_catalog_ordering_args', [$this, 'custom_ordering_args']);
            add_action('wp', [$this, 'remove_ordering_args']);
            add_filter('woocommerce_price_filter_sql', [$this, 'custom_price_filter_sql'], 10, 3);
            add_action('woocommerce_product_query', [$this, 'custom_price_filter_args']);
        }
        add_filter('update_post_metadata_cache', [$this, 'prime_table_caches'], 10, 2);
        add_action('clean_post_cache', [$this, 'clean_product_table_caches']);
    }
    ...
    {
        remove_filter('posts_clauses', [$this, 'custom_order_by_price_asc_post_clauses']);
        remove_filter('posts_clauses', [$this, 'custom_order_by_price_desc_post_clauses']);
        remove_filter('posts_clauses', [$this, 'custom_order_by_popularity_post_clauses']);
        remove_filter('posts_clauses', [$this, 'custom_order_by_rating_post_clauses']);
    }
    ...
    public function custom_ordering_args($args)
    {
        if ('price' === $args['orderby']) {
            if ('DESC' === $args['order']) {
                remove_filter('posts_clauses', [wc()->query, 'order_by_price_desc_post_clauses']);
                add_filter('posts_clauses', [$this, 'custom_order_by_price_desc_post_clauses']);
            } else {
                remove_filter('posts_clauses', [wc()->query, 'order_by_price_asc_post_clauses']);
                add_filter('posts_clauses', [$this, 'custom_order_by_price_asc_post_clauses']);
            }
        } elseif ('popularity' === $args['orderby']) {
            remove_filter('posts_clauses', [wc()->query, 'order_by_popularity_post_clauses']);
            add_filter('posts_clauses', [$this, 'custom_order_by_popularity_post_clauses']);
            unset($args['meta_key']);
        } elseif ('_wc_average_rating' === $args['meta_key']) {
            add_filter('posts_clauses', [$this, 'custom_order_by_rating_post_clauses']);
            unset($args['meta_key']);
        }
    ...
    public function custom_price_filter_sql($sql, $meta_query_sql, $tax_query_sql)
    {
        ...
        $sql .= " 	WHERE {$wpdb->posts}.post_type IN ('" . implode("','", array_map('esc_sql',
                apply_filters('woocommerce_price_filter_post_type', ['product', 'product_variation']))) . "')
     ...
     * (see JazzMan\WCProductTables\WC_Product_Tables_Query::custom_price_filter_post_clauses()).
     *
    ...
    public function custom_price_filter_args($wp_query)
    {
        ...
        if (isset($meta_query['price_filter']) && ! empty($meta_query['price_filter'])) {
            ...
            add_filter('posts_clauses', [$this, 'custom_price_filter_post_clauses'], 10, 2);
        }
    }
    ...
    public function prime_table_caches($return, $object_ids)
    {
        ...
        $rel_cache      = array_fill_keys($prime_ids, []);
        $download_cache = array_fill_keys($prime_ids, []);
    ...
    protected function prime_product_table_caches($prime_ids)
    {
        ...
        $att_cache       = array_fill_keys($prime_ids, []);
        $att_value_cache = array_fill_keys($prime_ids, []);
        ...
        foreach ($rows as $row) {
            if ( ! isset($att_value_cache[$row->product_id])) {
                $att_value_cache[$row->product_id] = [];
            }
    ...
    protected function prime_product_variation_table_caches($prime_ids)
    {
        ...
        $prime_id_sql        = implode(',', $prime_ids);
        $variation_att_cache = array_fill_keys($prime_ids, []);
        $rel_cache           = array_fill_keys($prime_ids, []);
        $download_cache      = array_fill_keys($prime_ids, []);
