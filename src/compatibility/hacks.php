...
{
    ?>
    <input type="hidden" name="attribute_product_attibute_ids[<?php echo esc_attr($i); ?>]"
           value="<?php echo esc_attr($attribute->get_product_attribute_id()); ?>"/>
    <?php
}

add_action('woocommerce_after_product_attribute_settings',
    'woocommerce_after_product_attribute_settings_custom_tables_support', 10, 2);
 ...
 * @param int                  $i         Index.
 *
 * @return \JazzMan\WCProductTables\WC_Product_Attribute
 */
...
add_filter('woocommerce_admin_meta_boxes_prepare_attribute',
    'woocommerce_admin_meta_boxes_prepare_attribute_custom_tables_support', 10, 3);
...
function woocommerce_product_custom_tables_custom_query_vars($args, $query)
{
    ...
    if (isset($query->query_vars['wc_products'])) {
        foreach ($query->query_vars['wc_products'] as $key => $value) {
            ...
            $args['where'] .= $wpdb->prepare(" AND wc_products.{$key} = %s ",
                $value); // WPCS: db call ok, unprepared sql ok.
        }
    }
