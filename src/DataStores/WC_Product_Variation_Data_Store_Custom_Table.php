...
class WC_Product_Variation_Data_Store_Custom_Table extends WC_Product_Data_Store_Custom_Table implements WC_Object_Data_Store_Interface
{
    ...
    */
    ...
    public function read(&$product)
    {
        ...
        if ( ! $product->get_id() || ! $post_object || ! in_array($post_object->post_type,
        ...
        if ($post_object->post_title !== $new_title) {
            ...
            $GLOBALS['wpdb']->update($GLOBALS['wpdb']->posts, ['post_title' => $new_title],
                ['ID' => $product->get_id()]);
    ...
    public function update(&$product)
    {
        ...
        if (array_intersect([
            ...
            $post_data = [
                ...
                'post_date'         => gmdate('Y-m-d H:i:s', $product->get_date_created('edit')->getOffsetTimestamp()),
                'post_date_gmt'     => gmdate('Y-m-d H:i:s', $product->get_date_created('edit')->getTimestamp()),
                'post_modified'     => isset($changes['date_modified']) ? gmdate('Y-m-d H:i:s',
                    $product->get_date_modified('edit')->getOffsetTimestamp()) : current_time('mysql'),
                'post_modified_gmt' => isset($changes['date_modified']) ? gmdate('Y-m-d H:i:s',
                    $product->get_date_modified('edit')->getTimestamp()) : current_time('mysql', 1),
                'post_type'         => 'product_variation',
                'post_name'         => $product->get_slug('edit'),
            ];
    ...
    */
    ...
    public function read_attributes(&$product)
    {
        ...
        $product_attributes = wp_cache_get('woocommerce_product_variation_attribute_values_' . $product->get_id(),
            'product');

        if (false === $product_attributes) {
            $product_attributes = $wpdb->get_results($wpdb->prepare("SELECT value, product_attribute_id FROM {$wpdb->prefix}wc_product_variation_attribute_values WHERE product_id = %d",
                $product->get_id())); // WPCS: db call ok, cache ok.

            wp_cache_set('woocommerce_product_variation_attribute_values_' . $product->get_id(), $product_attributes,
                'product');
        }
    ...
    */
    ...
    protected function generate_product_title($product)
    {
        ...
        $should_include_attributes = apply_filters('woocommerce_product_variation_title_include_attributes',
            $should_include_attributes, $product);
        $separator                 = apply_filters('woocommerce_product_variation_title_attributes_separator', ' - ',
            $product);
        $title_base                = get_post_field('post_title', $product->get_parent_id());
        $title_suffix              = $should_include_attributes ? wc_get_formatted_variation($product, true,
            false) : '';

        return apply_filters('woocommerce_product_variation_title',
            $title_suffix ? $title_base . $separator . $title_suffix : $title_base, $product, $title_base,
            $title_suffix);
    }
    ...
    protected function update_visibility(&$product, $force = false)
    {
        ...
        if ($force || array_intersect(['stock_status'], array_keys($changes))) {
            $terms = [];
    ...
    protected function get_parent_product_attribute_names(&$product)
    {
        ...
        $attributes = wp_cache_get('woocommerce_product_attributes_' . $product->get_parent_id(), 'product');

        if (false === $attributes) {
            $attributes = $wpdb->get_results($wpdb->prepare("SELECT * FROM {$wpdb->prefix}wc_product_attributes WHERE product_id = %d",
                $product->get_parent_id())); // WPCS: db call ok, cache ok.
    ...
    protected function update_attributes(&$product, $force = false)
    {
        ...
        if ($force || array_key_exists('attributes', $changes)) {
            ...
            $existing_attributes = wp_list_pluck($wpdb->get_results($wpdb->prepare("SELECT product_attribute_id, value FROM {$wpdb->prefix}wc_product_variation_attribute_values WHERE product_id = %d",
            ...
            if ($attributes) {
                ...
                foreach ($attributes as $attribute_key => $attribute_value) {
                    ...
                    if ( ! empty($existing_attributes[$product_attribute_id])) {
                        $wpdb->update("{$wpdb->prefix}wc_product_variation_attribute_values", [
                        ...
                        $wpdb->insert("{$wpdb->prefix}wc_product_variation_attribute_values", [
                            'value'                => $attribute_value,
                            'product_attribute_id' => $product_attribute_id,
                            'product_id'           => $product->get_id(),
                        ]); // WPCS: db call ok, cache ok.
                ...
                if ($attributes_to_delete) {
                    $wpdb->query("DELETE FROM {$wpdb->prefix}wc_product_variation_attribute_values WHERE product_id = " . absint($product->get_id()) . ' AND product_attribute_id IN (' . implode(',',
                            array_map('esc_sql',
                                $attributes_to_delete)) . ')'); // WPCS: db call ok, cache ok, unprepared SQL ok.
                }
            }
        }
    }
