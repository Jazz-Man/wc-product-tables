...
namespace JazzMan\WCProductTables\DataStores;

use DateTimeZone;
use Exception;
use JazzMan\WCProductTables\WC_Product_Attribute;
use WC_Comments;
...
/**
 * JazzMan\WCProductTables\DataStores\WC_Product_Data_Store_Custom_Table class.
 */
class WC_Product_Data_Store_Custom_Table extends WC_Data_Store_WP implements WC_Object_Data_Store_Interface, WC_Product_Data_Store_Interface
{
    ...
    protected function update_relationship(&$product, $type = '')
    {
        ...
        $relationships = array_filter($this->get_product_relationship_rows_from_db($product->get_id()),
        ...
        foreach ($new_values as $priority => $object_id) {
            $relationship = [
                'relationship_id' => $existing[$object_id] ?? 0,
                'type' => $type,
                'product_id' => $product->get_id(),
                'object_id' => $object_id,
                'priority' => $priority,
    ...
    protected function update_product_data(&$product)
    {
        ...
        foreach ($columns as $column) {
            if ($insert || array_key_exists($column, $changes)) {
                ...
                if (\in_array($column, $date_columns, true)) {
                    $data[$column] = empty($value) ? null : gmdate('Y-m-d H:i:s',
                        $product->{"get_$column"}('edit')->getOffsetTimestamp());
                } else {
        ...
        if ($insert) {
            ...
            $wpdb->update("{$wpdb->prefix}wc_products", $data, [
                'product_id' => $product->get_id(),
            ]); // WPCS: db call ok, cache ok.
        }
    ...
    protected function get_product_row_from_db($product_id)
    {
        ...
        if (false === $data) {
            $data = $wpdb->get_row($wpdb->prepare("SELECT * FROM {$wpdb->prefix}wc_products WHERE product_id = %d;",
                $product_id)); // WPCS: db call ok.
    ...
    protected function get_product_relationship_rows_from_db($product_id)
    {
        ...
        if (false === $data) {
            $data = $wpdb->get_results($wpdb->prepare("SELECT `relationship_id`, `object_id`, `type` FROM {$wpdb->prefix}wc_product_relationships WHERE `product_id` = %d ORDER BY `priority` ASC",
                $product_id)); // WPCS: db call ok.
    ...
    protected function get_product_downloads_rows_from_db($product_id)
    {
        ...
        if (false === $data) {
            $data = $wpdb->get_results($wpdb->prepare("SELECT `download_id`, `name`, `file`, `priority` FROM {$wpdb->prefix}wc_product_downloads WHERE `product_id` = %d ORDER BY `priority` ASC",
                $product_id)); // WPCS: db call ok.
    ...
    protected function read_product_data(&$product)
    {
        ...
        $props['manage_stock'] = isset($props['stock_quantity']) && null !== $props['stock_quantity'];
    ...
    public function update(&$product)
    {
        ...
        if (array_intersect([
            ...
            if ($product->get_date_created('edit')) {
                $post_data['post_date'] = gmdate('Y-m-d H:i:s',
                    $product->get_date_created('edit')->getOffsetTimestamp());
            ...
            if (isset($changes['date_modified']) && $product->get_date_modified('edit')) {
                $post_data['post_modified'] = gmdate('Y-m-d H:i:s',
                    $product->get_date_modified('edit')->getOffsetTimestamp());
                $post_data['post_modified_gmt'] = gmdate('Y-m-d H:i:s',
                    $product->get_date_modified('edit')->getTimestamp());
            } else {
            ...
            if (doing_action('save_post')) {
                $GLOBALS['wpdb']->update($GLOBALS['wpdb']->posts, $post_data, [
                    'ID' => $product->get_id(),
                ]);
    ...
    public function delete(&$product, $args = [])
    {
        ...
        if ($args['force_delete']) {
            $wpdb->delete("{$wpdb->prefix}wc_products", ['product_id' => $id], ['%d']); // WPCS: db call ok, cache ok.
            $wpdb->delete("{$wpdb->prefix}wc_product_relationships", ['product_id' => $id],
                ['%d']); // WPCS: db call ok, cache ok.
            $wpdb->delete("{$wpdb->prefix}wc_product_downloads", ['product_id' => $id],
                ['%d']); // WPCS: db call ok, cache ok.
            $wpdb->delete("{$wpdb->prefix}wc_product_variation_attribute_values", ['product_id' => $id],
                ['%d']); // WPCS: db call ok, cache ok.
            $wpdb->delete("{$wpdb->prefix}wc_product_attribute_values", ['product_id' => $id],
                ['%d']); // WPCS: db call ok, cache ok.
            wp_delete_post($id);
            $product->set_id(0);
    ...
    protected function handle_updated_props(&$product)
    {
        ...
        if (\in_array('regular_price', $this->updated_props, true) || \in_array('sale_price', $this->updated_props,
                true)) {
            if ($product->get_sale_price('edit') >= $product->get_regular_price('edit')) {
                $wpdb->update("{$wpdb->prefix}wc_products", [
                    'sale_price' => null,
                ], [
                    'product_id' => $product->get_id(),
                ]); // WPCS: db call ok, cache ok.
        ...
        if (\in_array('date_on_sale_from', $this->updated_props, true) || \in_array('date_on_sale_to',
                $this->updated_props, true) || \in_array('regular_price', $this->updated_props,
                true) || \in_array('sale_price', $this->updated_props, true) || \in_array('product_type',
                $this->updated_props, true)) {
        ...
        if (\in_array('stock_quantity', $this->updated_props, true)) {
            do_action($product->is_type('variation') ? 'woocommerce_variation_set_stock' : 'woocommerce_product_set_stock',
                $product);
        }

        if (\in_array('stock_status', $this->updated_props, true)) {
            do_action($product->is_type('variation') ? 'woocommerce_variation_set_stock_status' : 'woocommerce_product_set_stock_status',
                $product->get_id(), $product->get_stock_status(), $product);
        }
    ...
    public function get_on_sale_products()
    {
        ...
        return $wpdb->get_results("SELECT products.product_id as id, posts.post_parent as parent_id
			...
			AND products.price = products.sale_price"); // WPCS: db call ok, cache ok.
    }
    ...
    public function is_existing_sku($product_id, $sku)
    {
        ...
        return $wpdb->get_var($wpdb->prepare("SELECT products.product_id
				FROM {$wpdb->prefix}wc_products as products
				LEFT JOIN {$wpdb->posts} as posts ON products.product_id = posts.ID
				WHERE posts.post_status != 'trash'
				AND products.sku = %s
				AND products.product_id <> %d
				LIMIT 1", wp_slash($sku), $product_id)); // WPCS: db call ok, cache ok.
    }
    ...
    public function get_product_id_by_sku($sku)
    {
        ...
        $id = $wpdb->get_var($wpdb->prepare("SELECT products.product_id
				FROM {$wpdb->prefix}wc_products as products
				LEFT JOIN {$wpdb->posts} as posts ON products.product_id = posts.ID
				WHERE posts.post_status != 'trash'
				AND products.sku = %s
				LIMIT 1", $sku)); // WPCS: db call ok, cache ok.
    ...
    public function get_starting_sales()
    {
        ...
        return $wpdb->get_col($wpdb->prepare("SELECT products.product_id
				FROM {$wpdb->prefix}wc_products as products
				LEFT JOIN {$wpdb->posts} as posts ON products.product_id = posts.ID
    ...
    public function find_matching_product_variation($product, $match_attributes = [])
    {
        ...
        if ($variations && !is_wp_error($variations)) {
             ...
             */
            return (array_map('sanitize_title',
                    $match_attributes) === $match_attributes) ? 0 : $this->find_matching_product_variation($product,
                array_map('sanitize_title', $match_attributes));
        }
    ...
    public function sort_all_product_variations($parent_id)
    {
        ...
        $ids = $wpdb->get_col($wpdb->prepare("SELECT ID FROM {$wpdb->posts} WHERE post_type = 'product_variation' AND post_parent = %d AND post_status = 'publish' ORDER BY menu_order ASC, ID ASC",
            $parent_id));
        $index = 1;
    ...
    public function get_related_products($cats_array, $tags_array, $exclude_ids, $limit, $product_id)
    {
        ...
        $related_product_query = (array) apply_filters('woocommerce_product_related_posts_query',
            $this->get_related_products_query($cats_array, $tags_array, $exclude_ids, $limit + 10), $product_id, $args);
    ...
    public function get_related_products_query($cats_array, $tags_array, $exclude_ids, $limit)
    {
        ...
        if (\count($exclude_term_ids)) {
            $query['join'] .= " LEFT JOIN ( SELECT object_id FROM {$wpdb->term_relationships} WHERE term_taxonomy_id IN ( ".implode(',',
                    array_map('absint', $exclude_term_ids)).' ) ) AS exclude_join ON exclude_join.object_id = p.ID';
            $query['where'] .= ' AND exclude_join.object_id IS NULL';
        }

        if (\count($include_term_ids)) {
            $query['join'] .= " INNER JOIN ( SELECT object_id FROM {$wpdb->term_relationships} INNER JOIN {$wpdb->term_taxonomy} using( term_taxonomy_id ) WHERE term_id IN ( ".implode(',',
                    array_map('absint', $include_term_ids)).' ) ) AS include_join ON include_join.object_id = p.ID';
        }
    ...
    public function update_product_stock($product_id_with_stock, $stock_quantity = null, $operation = 'set')
    {
        ...
        switch ($operation) {
            case 'increase':
                $wpdb->query($wpdb->prepare("UPDATE {$wpdb->prefix}wc_products SET stock_quantity = stock_quantity + %f WHERE product_id = %d;",
                    $stock_quantity, $product_id_with_stock)); // WPCS: db call ok, cache ok.
                break;
            case 'decrease':
                $wpdb->query($wpdb->prepare("UPDATE {$wpdb->prefix}wc_products SET stock_quantity = stock_quantity - %f WHERE product_id = %d;",
                    $stock_quantity, $product_id_with_stock)); // WPCS: db call ok, cache ok.
                break;
            default:
                $wpdb->query($wpdb->prepare("UPDATE {$wpdb->prefix}wc_products SET stock_quantity = %f WHERE product_id = %d;",
                    $stock_quantity, $product_id_with_stock)); // WPCS: db call ok, cache ok.
                break;
        }
    ...
    public function update_product_sales($product_id, $quantity = null, $operation = 'set')
    {
        ...
        switch ($operation) {
            case 'increase':
                $wpdb->query($wpdb->prepare("UPDATE {$wpdb->prefix}wc_products SET total_sales = total_sales + %f WHERE product_id = %d;",
                    $quantity, $product_id)); // WPCS: db call ok, cache ok.
                break;
            case 'decrease':
                $wpdb->query($wpdb->prepare("UPDATE {$wpdb->prefix}wc_products SET total_sales = total_sales - %f WHERE product_id = %d;",
                    $quantity, $product_id)); // WPCS: db call ok, cache ok.
                break;
            default:
                $wpdb->query($wpdb->prepare("UPDATE {$wpdb->prefix}wc_products SET total_sales = %f WHERE product_id = %d;",
                    $quantity, $product_id)); // WPCS: db call ok, cache ok.
                break;
        }
    ...
    {
        global $wpdb;
        $wpdb->query($wpdb->prepare("UPDATE {$wpdb->prefix}wc_products SET average_rating = %f WHERE product_id = %d;",
            $product->get_average_rating('edit'), $product->get_id())); // WPCS: db call ok, cache ok.
    ...
    protected function read_downloads(&$product)
    {
        ...
        if ($existing_downloads) {
            ...
            foreach ($existing_downloads as $data) {
                ...
                $download->set_file(apply_filters('woocommerce_file_download_path', $data->file, $product,
                    $data->download_id));
                $downloads[] = $download;
            }
    ...
    protected function update_downloads(&$product, $force = false)
    {
        ...
        if ($force || array_key_exists('downloads', $changes)) {
            ...
            if ($product->is_type('variation')) {
                do_action('woocommerce_process_product_file_download_paths', $product->get_parent_id(),
                    $product->get_id(), $downloads);
            } else {
                do_action('woocommerce_process_product_file_download_paths', $product->get_id(), 0, $downloads);
            }

            if ($downloads) {
                $existing_downloads = array_filter(array_map('absint',
                    wp_list_pluck($this->get_product_downloads_rows_from_db($product->get_id()), 'download_id')));
                $updated = [];

                foreach (array_values($downloads) as $key => $data) {
                    $download_id = 'tmp_' === substr($data['id'], 0, 4) ? 0 : (int) ($data['id']);
                    $download = [
                ...
                if ($missing) {
                    $wpdb->query("DELETE FROM {$wpdb->prefix}wc_product_downloads WHERE `download_id` IN ( '".implode("','",
                            $missing)."' )"); // WPCS: db call ok, cache ok, unprepared SQL ok.
                }
            } else {
                $wpdb->query($wpdb->prepare("DELETE FROM {$wpdb->prefix}wc_product_downloads WHERE `product_id` = %d",
    ...
    public function read_attributes(&$product)
    {
        ...
        if (false === $product_attributes) {
            $product_attributes = $wpdb->get_results($wpdb->prepare("SELECT * FROM {$wpdb->prefix}wc_product_attributes WHERE product_id = %d",
                $product->get_id())); // WPCS: db call ok, cache ok.
        ...
        if (!empty($product_attributes)) {
            $attributes = [];
            $default_attributes = [];
            $cached_attr_value_data = wp_cache_get('woocommerce_product_attribute_values_'.$product->get_id(),
                'product');
            ...
            foreach ($product_attributes as $attr) {
                ...
                if (!isset($cached_attr_value_data[$attr->product_attribute_id])) {
                    $cached_attr_value_data[$attr->product_attribute_id] = $wpdb->get_results($wpdb->prepare("SELECT value, is_default FROM {$wpdb->prefix}wc_product_attribute_values WHERE product_attribute_id = %d",
            ...
            wp_cache_set('woocommerce_product_attribute_values_'.$product->get_id(), $cached_attr_value_data,
                'product');
    ...
    protected function update_terms(&$product, $force = false)
    {
        ...
        if ($force || array_key_exists('category_ids', $changes)) {
            ...
            wp_set_post_terms($product->get_id(), $categories, 'product_cat');
        }
        if ($force || array_key_exists('tag_ids', $changes)) {
            wp_set_post_terms($product->get_id(), $product->get_tag_ids('edit'), 'product_tag');
        }
        if ($force || array_key_exists('shipping_class_id', $changes)) {
            wp_set_post_terms($product->get_id(), [$product->get_shipping_class_id('edit')], 'product_shipping_class');
        }
    }
    ...
    protected function update_visibility(&$product, $force = false)
    {
        ...
        if ($force || array_intersect(['featured', 'stock_status', 'average_rating', 'catalog_visibility'],
                array_keys($changes))) {
            $terms = [];
            ...
            if (!is_wp_error(wp_set_post_terms($product->get_id(), $terms, 'product_visibility'))) {
    ...
    protected function update_attributes(&$product, $force = false)
    {
        ...
        if ($force || array_key_exists('attributes', $changes) || array_key_exists('default_attributes', $changes)) {
            ...
            $existing_attributes = wp_list_pluck($wpdb->get_results($wpdb->prepare("SELECT product_attribute_id, attribute_id FROM {$wpdb->prefix}wc_product_attributes WHERE product_id = %d",
                $product->get_id())), 'attribute_id', 'product_attribute_id'); // WPCS: db call ok, cache ok.
            $updated_attributes = [];
            ...
            if ($attributes) {
                foreach ($attributes as $attribute_key => $attribute) {
                    ...
                    $attribute_values = $attribute->is_taxonomy() ? wp_list_pluck($attribute->get_terms(),
                        'term_id') : $attribute->get_options();
                    ...
                    if ($product_attribute_id) {
                        $wpdb->update("{$wpdb->prefix}wc_product_attributes", $attribute_data, [
                    ...
                    // Get existing values.
                    $existing_attribute_values = array_map('absint',
                        wp_list_pluck($wpdb->get_results($wpdb->prepare("SELECT attribute_value_id, value FROM {$wpdb->prefix}wc_product_attribute_values WHERE product_attribute_id = %d AND product_id = %d",
                            $product_attribute_id, $product->get_id())), 'value',
                            'attribute_value_id')); // WPCS: db call ok, cache ok.
                    ...
                    if ($attributes_values_to_delete) {
                        $wpdb->query("DELETE FROM {$wpdb->prefix}wc_product_attribute_values WHERE attribute_value_id IN (".implode(',',
                                array_map('esc_sql',
                                    array_keys($attributes_values_to_delete))).')'); // WPCS: db call ok, cache ok, unprepared SQL ok.
                    }
                    ...
                    foreach ($attribute_values as $attribute_value) {
                        ...
                        if ($attribute_value_id) {
                            $wpdb->update("{$wpdb->prefix}wc_product_attribute_values", $attribute_value_data, [
                            ...
                            $wpdb->insert("{$wpdb->prefix}wc_product_attribute_values", $attribute_value_data, [
                                '%d',
                                '%d',
                                '%s',
                                '%d',
                    ...
                    if ($attribute->is_taxonomy()) {
                        wp_set_object_terms($product->get_id(), wp_list_pluck($attribute->get_terms(), 'term_id'),
                            $attribute->get_name());
                    }
            ...
            if ($attributes_to_delete) {
                $wpdb->query("DELETE FROM {$wpdb->prefix}wc_product_attributes WHERE product_attribute_id IN (".implode(',',
                        array_map('esc_sql',
                            array_keys($attributes_to_delete))).')'); // WPCS: db call ok, cache ok, unprepared SQL ok.
                ...
                foreach ($attributes_to_delete as $product_attribute_id => $attribute_id) {
                    ...
                    if (taxonomy_exists($taxonomy)) {
                        ...
                        wp_set_object_terms($product->get_id(), [], $taxonomy);
                    }
                }
            }
    ...
    public function search_products($term, $type = '', $include_variations = false, $all_statuses = false)
    {
        ...
        foreach ($term_groups as $term_group) {
            ...
            foreach ($search_terms as $search_term) {
                ...
                $term_group_query .= $wpdb->prepare(" {$searchand} ( ( posts.post_title LIKE %s) OR ( posts.post_excerpt LIKE %s) OR ( posts.post_content LIKE %s ) OR ( products.sku LIKE %s ) )",
        ...
        $search_results = $wpdb->get_results("SELECT DISTINCT posts.ID as product_id, posts.post_parent as parent_id FROM {$wpdb->posts} posts
        ...
        $product_ids = wp_parse_id_list(array_merge(wp_list_pluck($search_results, 'product_id'),
            wp_list_pluck($search_results, 'parent_id')));
    ...
    protected function get_wp_query_args($query_vars)
    {
        ...
        $wp_query_args = wp_parse_args(parent::get_wp_query_args($query_vars), [
            'date_query' => [],
            'meta_query' => [], // @codingStandardsIgnoreLine.
            'wc_products_query' => [], // Custom table queries will be stored here and turned into queries later.
        ]);
        ...
        if (isset($query_vars['date_on_sale_from']) && '' !== $query_vars['date_on_sale_from']) {
            $wp_query_args = $this->parse_date_for_wp_query($query_vars['date_on_sale_from'], 'date_on_sale_from',
                $wp_query_args);
            unset($wp_query_args['date_on_sale_from']);
        }

        if (isset($query_vars['date_on_sale_to']) && '' !== $query_vars['date_on_sale_to']) {
            $wp_query_args = $this->parse_date_for_wp_query($query_vars['date_on_sale_to'], 'date_on_sale_to',
                $wp_query_args);
        ...
        if (isset($query_vars['manage_stock']) && '' !== $query_vars['manage_stock'] && ! isset($wp_query_args['wc_products_query']['stock_quantity'])) {
            $wp_query_args['wc_products_query']['stock_quantity'] = [
        ...
        return apply_filters('woocommerce_product_data_store_cpt_get_products_query', $wp_query_args, $query_vars,
            $this);
    }
    ...
    public function parse_date_for_wp_query($query_var, $key, $wp_query_args = [])
    {
        ...
        try {
            ...
            } elseif (preg_match($query_parse_regex, $query_var,
                $sections)) { // Query with operators and possible range of dates.
                if (!empty($sections[1])) {
                    $dates[] = is_numeric($sections[1]) ? new WC_DateTime("@{$sections[1]}",
                        new DateTimeZone('UTC')) : wc_string_to_datetime($sections[1]);
                }

                $operator = \in_array($sections[2], $valid_operators, true) ? $sections[2] : '';
                $dates[] = is_numeric($sections[3]) ? new WC_DateTime("@{$sections[3]}",
                    new DateTimeZone('UTC')) : wc_string_to_datetime($sections[3]);
        ...
        if (\in_array($key, ['post_date', 'post_modified', 'date_on_sale_from', 'date_on_sale_to'], true)) {
        ...
        if (!isset($wp_query_args['meta_query'])) {
            $wp_query_args['meta_query'] = []; // phpcs:ignore WordPress.VIP.SlowDBQuery.slow_db_query_meta_query
        }
        ...
        if ('day' === $precision) {
            ...
            $end_timestamp = '...' !== $operator ? ($start_timestamp + DAY_IN_SECONDS) : strtotime(gmdate('m/d/Y 00:00:00',
        ...
        if (!empty($query->query_vars['wc_products_query'])) {
            foreach ($query->query_vars['wc_products_query'] as $name => $query) {
                ...
                switch ($compare) {
                    ...
                    case 'IN':
                    case 'NOT IN':
                        $where .= " AND products.`{$name}` {$compare} ('".implode("','",
                                array_map('esc_sql', $value))."') ";
                        break;
                    case 'LIKE':
                        // phpcs:ignore WordPress.WP.PreparedSQL.NotPrepared, WordPress.DB.PreparedSQLPlaceholders.UnfinishedPrepare
                        $where .= $wpdb->prepare(" AND products.`{$name}` LIKE {$format} ",
                            '%'.$wpdb->esc_like($value).'%');
                        break;
                    default:
                        // phpcs:ignore WordPress.WP.PreparedSQL.NotPrepared, WordPress.DB.PreparedSQLPlaceholders.UnfinishedPrepare
    ...
    public function query($query_vars)
    {
        ...
        if (!empty($args['errors'])) {
            ...
            add_filter('posts_join', [$this, 'products_join'], 10);
            add_filter('posts_where', [$this, 'products_where'], 10, 2);
            $query = new WP_Query($args);
            remove_filter('posts_join', [$this, 'products_join']);
            remove_filter('posts_where', [$this, 'products_where'], 10, 2);
        }
        ...
        $products = (isset($query_vars['return']) && 'ids' === $query_vars['return']) ? $query->posts : array_filter(array_map('wc_get_product',
            $query->posts));
