 ...
 */
class WC_Product_Variable_Data_Store_Custom_Table extends WC_Product_Data_Store_Custom_Table
{
    ...
    public function read_children(&$product, $force_read = false)
    {
        ...
        if (empty($children) || ! is_array($children) || ! isset($children['all']) || ! isset($children['visible']) || $force_read) {
            ...
            $children['all']     = wc_get_products(apply_filters('woocommerce_variable_children_args',
                $this->map_legacy_product_args($all_args), $product, false));
            $children['visible'] = wc_get_products(apply_filters('woocommerce_variable_children_args',
                $this->map_legacy_product_args($visible_only_args), $product, true));
        ...
        if ( ! empty($attributes)) {
            foreach ($attributes as $attribute) {
                ...
                if ( ! empty($child_ids)) {
                    ...
                    $query_args           = ['product_attribute_id' => $product_attribute_id] + $child_ids;
                    $values               = array_unique($wpdb->get_col($wpdb->prepare( // wpcs: PreparedSQLPlaceholders replacement count ok.
                        "
    ...
    /**
     * Get an array of all sale and regular prices from all variations. This is used for example when displaying the
     * price range at variable product level or seeing if the variable product is on sale.
     *
     * Can be filtered by plugins which modify costs, but otherwise will include the raw meta costs unlike get_price()
     * which runs costs through the woocommerce_get_price filter. This is to ensure modified prices are not cached,
     * unless intended.
     *
     * @since  3.0.0
     *
     * @param  WC_Product $product     Product object.
     * @param  bool       $for_display If true, prices will be adapted for display based on the
     *                                 `woocommerce_tax_display_shop` setting (including or excluding taxes).
     *
    ...
    public function read_price_data(&$product, $for_display = false)
    {
        ...
        if (empty($this->prices_array[$price_hash])) {
            $transient_cached_prices_array = array_filter((array)json_decode(strval(get_transient($transient_name)),
                true));
            ...
            if (empty($transient_cached_prices_array['version']) || WC_Cache_Helper::get_transient_version('product') !== $transient_cached_prices_array['version']) {
                $transient_cached_prices_array = ['version' => WC_Cache_Helper::get_transient_version('product')];
            }
            ...
            if (empty($transient_cached_prices_array[$price_hash])) {
                ...
                foreach ($variation_ids as $variation_id) {
                    ...
                    if ($variation) {
                        $price         = apply_filters('woocommerce_variation_prices_price',
                            $variation->get_price('edit'), $variation, $product);
                        $regular_price = apply_filters('woocommerce_variation_prices_regular_price',
                            $variation->get_regular_price('edit'), $variation, $product);
                        $sale_price    = apply_filters('woocommerce_variation_prices_sale_price',
                            $variation->get_sale_price('edit'), $variation, $product);
                        ...
                        if ($for_display) {
                            if ('incl' === get_option('woocommerce_tax_display_shop')) {
                                $price         = '' === $price ? '' : wc_get_price_including_tax($variation, [
                                    'qty'   => 1,
                                    'price' => $price,
                                ]);
                                $regular_price = '' === $regular_price ? '' : wc_get_price_including_tax($variation, [
                                    'qty'   => 1,
                                    'price' => $regular_price,
                                ]);
                                $sale_price    = '' === $sale_price ? '' : wc_get_price_including_tax($variation, [
                                    'qty'   => 1,
                                    'price' => $sale_price,
                                ]);
                            } else {
                                $price         = '' === $price ? '' : wc_get_price_excluding_tax($variation, [
                                    'qty'   => 1,
                                    'price' => $price,
                                ]);
                                $regular_price = '' === $regular_price ? '' : wc_get_price_excluding_tax($variation, [
                                    'qty'   => 1,
                                    'price' => $regular_price,
                                ]);
                                $sale_price    = '' === $sale_price ? '' : wc_get_price_excluding_tax($variation, [
                                    'qty'   => 1,
                                    'price' => $sale_price,
                                ]);
                            }
                        ...
                        $prices_array['price'][$variation_id]         = wc_format_decimal($price,
                            wc_get_price_decimals());
                        $prices_array['regular_price'][$variation_id] = wc_format_decimal($regular_price,
                            wc_get_price_decimals());
                        $prices_array['sale_price'][$variation_id]    = wc_format_decimal($sale_price . '.00',
                            wc_get_price_decimals());

                        $prices_array = apply_filters('woocommerce_variation_prices_array', $prices_array, $variation,
                            $for_display);
                    }
                }
             ...
             */
            $this->prices_array[$price_hash] = apply_filters('woocommerce_variation_prices',
    ...
    /**
     * Create unique cache key based on the tax location (affects displayed/cached prices), product version and active
     * price filters. DEVELOPERS should filter this hash if offering conditional pricing to keep it unique.
     *
     * @since  3.0.0
     *
     * @param  WC_Product $product       Product object.
    ...
    protected function get_price_hash(&$product, $include_taxes = false)
    {
        ...
        $price_hash   = md5(wp_json_encode(apply_filters('woocommerce_get_variation_prices_hash', $price_hash, $product,
            $include_taxes)));
    ...
    public function child_has_weight($product)
    {
        ...
        if (false === $child_has_weight) {
            ...
            $child_has_weight = null !== $wpdb->get_var($wpdb->prepare($query,
                $product->get_id())) ? 1 : 0; // WPCS: db call ok, cache ok, unprepared SQL OK.
    ...
    public function child_has_dimensions($product)
    {
        ...
        $child_has_dimensions = wp_cache_get('woocommerce_product_child_has_dimensions_' . $product->get_id(),
            'product');
        ...
        if (false === $child_has_dimensions) {
            ...
            $child_has_dimensions = null !== $wpdb->get_var($wpdb->prepare($query,
                $product->get_id())) ? 1 : 0; // WPCS: db call ok, cache ok, unprepared SQL OK.

            wp_cache_set('woocommerce_product_child_has_dimensions_' . $product->get_id(), $child_has_dimensions,
                'product');
        }
    ...
    public function child_has_stock_status($product, $status)
    {
        ...
        $children_stock_status = wp_cache_get('woocommerce_product_children_stock_status_' . $product->get_id(),
            'product');

        if (false === $children_stock_status) {
            $children_stock_status = [];
        }

        if ( ! isset($children_stock_status[$status])) {
            $children_stock_status[$status] = (bool)$wpdb->get_var($wpdb->prepare("SELECT product_id
					FROM {$wpdb->prefix}wc_products as products
					LEFT JOIN {$wpdb->posts} as posts ON products.product_id = posts.ID
					WHERE posts.post_parent = %d
					AND products.stock_status = %s
					LIMIT 1", $product->get_id(), $status));

            wp_cache_set('woocommerce_product_children_stock_status_' . $product->get_id(), $children_stock_status,
                'product');
        }
    ...
    public function sync_variation_names(&$product, $previous_name = '', $new_name = '')
    {
        if ($new_name !== $previous_name) {
            ...
            $wpdb->query($wpdb->prepare("UPDATE {$wpdb->posts}
					...
					AND post_parent = %d", $previous_name ? $previous_name : 'AUTO-DRAFT', $new_name,
                $product->get_id()));
        }
    }
    ...
    public function sync_managed_variation_stock_status(&$product)
    {
        ...
        if ($product->get_manage_stock()) {
            ...
            if ( ! empty($children)) {
                $changed = $wpdb->query($wpdb->prepare("UPDATE {$wpdb->prefix}wc_products
						SET stock_status = %s
    ...
    public function sync_price(&$product)
    {
        ...
        $min_price = $children ? $wpdb->get_var("SELECT price FROM {$wpdb->prefix}wc_products WHERE product_id IN (" . implode(',',
                array_map('absint',
                    $children)) . ') ORDER BY price ASC LIMIT 1') : null; // phpcs:ignore WordPress.WP.PreparedSQL.NotPrepared

        if ( ! is_null($min_price)) {
            $wpdb->query($wpdb->prepare("UPDATE {$wpdb->prefix}wc_products
					SET price = %d
					WHERE product_id = %d", wc_format_decimal($min_price), $product->get_id()));
        }
    }
    ...
    public function delete_variations($product_id, $force_delete = false)
    {
        ...
        $variation_ids = wp_parse_id_list(get_posts([
        ...
        if ( ! empty($variation_ids)) {
            foreach ($variation_ids as $variation_id) {
                if ($force_delete) {
                    ...
                    $wpdb->delete("{$wpdb->prefix}wc_products", ['product_id' => $variation_id], ['%d']);
                } else {
