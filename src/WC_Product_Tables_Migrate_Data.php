...
use stdClass;
use WC_Product;
use JazzMan\WCProductTables\WC_Product_Tables_Backwards_Compatibility;

/**
 * Class JazzMan\WCProductTables\WC_Product_Tables_Migrate_Data
 */
class WC_Product_Tables_Migrate_Data
{
    ...
    public static function migrate_product($product, $clean_old_data = false)
    {
        ...
        $downloadable_files = isset($metas['_downloadable_files']) ? maybe_unserialize($metas['_downloadable_files'][0]) : [];
    ...
    public static function get_products($post_type = false)
    {
        ...
        // phpcs:disable
        return $wpdb->get_results("SELECT ID, post_type FROM {$wpdb->posts}
    ...
    protected static function migrate_variation_attribute_values($parent_id, $attribute_id, $attribute_name)
    {
        ...
        $variable_products = $wpdb->get_results($wpdb->prepare("SELECT * FROM {$wpdb->posts} WHERE post_type = 'product_variation' AND post_parent = %d",
            $parent_id));
     ...
     */
    protected static function migrate_custom_attributes(
        $product_id,
    ...
    protected static function clean_old_data($product_id)
    {
        ...
        $wpdb->query($wpdb->prepare("DELETE FROM {$wpdb->postmeta} WHERE post_id = %d AND meta_key IN ({$meta_keys_in})",
            // phpcs:ignore
            $product_id));

        $wpdb->query($wpdb->prepare("DELETE FROM {$wpdb->postmeta} WHERE post_id = %d AND meta_key LIKE %s",
            $product_id, $wpdb->esc_like('attribute_') . '%'));
